...
__attribute__((naked))
__attribute__((aligned(4))) // function starts at an addr divisible by 4 (risc-v instructions must be 4 byte aligned)
void kernel_entry(void) {
  __asm__ __volatile__(
    "csrw sscratch, sp\n" // saves stack pointer (sp) to sscratch (for using it in saving cpu state)
    "addi sp, sp, -4 * 31\n" // allocates 124 bytes of stack memory (31 register, each 4 byte)
    "sw ra, 4 * 0(sp)\n" // store word (sw) ra , 4 bytes from stack top
    "sw gp,  4 * 1(sp)\n"
    "sw tp,  4 * 2(sp)\n"
    "sw t0,  4 * 3(sp)\n"
    "sw t1,  4 * 4(sp)\n"
    ...
    "csrr a0, sscratch\n"
    "sw a0, 4*30(sp)\n" // save original stack pointer in the stack
...
// boot section defined in the kernel.ld
// kernel code starts from here
// void boot() marks the entry point in kernel.ld
...
void boot(void) {
  __asm__ __volatile__(
    ...
    :
    : [stack_top] "r" (__stack_top) // pass stack_top adress into stack_top variable
  );
}
